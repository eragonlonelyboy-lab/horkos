...
const { lifecycleFromCommand } = require('./claims');
...
function recordFromHook(hookInput, thresholds) {
  ...
  if (!cls) {
    // Lifecycle rows: deletes and moves change where a ledgered write's artifact
    // lives. They must be recorded HERE, not only parsed from the transcript at
    // audit time, because PostToolUse fires for subagent tool calls while the Stop
    // hook only receives the main session's transcript. A subagent that deletes or
    // moves a truthfully written file was invisible before this (dogfood
    // 2026-07-10, Monara devScene.js; dogfood 2026-07-09, bluwaterstudio mv).
    // Failure-safe by construction: a delete row only ever excuses a file that is
    // ALREADY missing, and a move row only redirects verification to a destination
    // that must itself pass. Neither can weaken a check on a file that exists.
    const n = String(hookInput.tool_name || '');
    if (n === 'Bash' || n === 'PowerShell') {
      const lc = lifecycleFromCommand(String((hookInput.tool_input || {}).command || ''));
      const rows = [];
      const ts = new Date().toISOString();
      for (const d of lc.deleted) rows.push({ ts, tool: n, system: 'fs', op: 'delete', target: d.path, recursive: d.recursive, lifecycle: true, receipt: { ok: true } });
      for (const m of lc.moves) rows.push({ ts, tool: n, system: 'fs', op: 'move', target: m.from, moved_to: m.to, lifecycle: true, receipt: { ok: true } });
      for (const r of rows) appendJSONL(ledgerPath(hookInput.session_id), r);
      return rows.length ? rows : null;
    }
    return null; // not a tracked write: get out fast
  }
