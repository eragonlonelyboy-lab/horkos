...
function classifyTool(toolName, toolInput) {
  ...
  if (n === 'Bash' || n === 'PowerShell') {
    ...
    if (/\bgit\s+(commit|push|tag|merge)\b/.test(cmd)) {
      const op = RegExp.$1;
      // Record WHICH repo: a leading `cd <path>` in the command is the best signal
      // (wrong-repo false positive found in production dogfood 2026-07-05: the
      // audit checked HEAD age in the hook's cwd, not the committed repo).
      const cd = cmd.match(/(?:^|&&|;)\s*cd\s+(?:"([^"]+)"|'([^']+)'|([^\s;&|]+))/);
      return { system: 'git', op, target: (cd && (cd[1] || cd[2] || cd[3])) || null };
    }
    return null;
  }
