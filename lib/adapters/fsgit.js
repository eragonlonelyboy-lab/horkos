...
async function verify(entry, tier, config, context) {
  if (entry.system === 'fs') {
    ...
    if (!fs.existsSync(p)) {
      ...
      // delete command naming this exact path, or a RECURSIVE delete of an ancestor
      // directory of it. That is a truthful write followed by cleanup, not a phantom
      // (which has no ledger entry) or a lie. (FP class 7, dogfood 2026-07-06: a stray
      // launch.json Written then removed with `rm`. FP class 12, dogfood 2026-07-09:
      // `rm -rf tools/coverage` after folding that tree into the product.)
      if (context && context.intentionallyDeleted && entry.receipt && entry.receipt.ok) {
        return { status: 'pass', detail: `written then intentionally deleted (the transcript deletes this path, or recursively deletes a directory above it); the write claim was truthful: ${p}` };
      }
    ...
    if (entry.sent_head && (entry.op === 'create-or-overwrite' || entry.tool === 'Edit')) {
      ...
      if (probe && !content.includes(probe)) {
        // The head probe carries a hidden premise: that nothing touched this file
        // after the recorded write. A LATER LEDGERED write is already handled by
        // supersession in audit.js, but an out-of-band rewrite (a shell command, a
        // script, a formatter) leaves no ledger entry, so the write-time head is no
        // longer expected to survive. mtime settles it deterministically.
        //
        // Tight, not blanket: a phantom (no file) and an empty file already failed
        // above, and a LYING write that nothing touched afterwards still fails here,
        // because its mtime sits at the write timestamp. Only a truthful write that
        // was legitimately edited later is spared. (FP class 11, dogfood 2026-07-08:
        // an em-dash cleanup script rewrote line 2 of seven files it had just Written.)
        const SKEW_MS = 2000;
        const wroteAt = entry.ts ? Date.parse(entry.ts) : NaN;
        const modifiedAfterWrite = Number.isFinite(wroteAt) && stat.mtimeMs > wroteAt + SKEW_MS;
        if (modifiedAfterWrite) {
          return {
            status: 'pass',
            detail: `write was truthful (file exists, ${stat.size} bytes); head probe not applicable: file was modified ${Math.round((stat.mtimeMs - wroteAt) / 1000)}s after the recorded write by something outside the ledger, so content is unverified: ${p}`,
          };
        }
