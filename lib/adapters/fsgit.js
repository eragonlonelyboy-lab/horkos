...
async function verify(entry /*, tier, config */) {
  ...
  if (entry.system === 'git') {
    try {
      const head = execFileSync('git', ['rev-parse', 'HEAD'], { cwd: process.cwd(), encoding: 'utf8', timeout: 5000 }).trim();
      if (entry.op === 'commit') {
        const msgTs = execFileSync('git', ['log', '-1', '--format=%cI'], { cwd: process.cwd(), encoding: 'utf8', timeout: 5000 }).trim();
        const ageMs = Date.now() - new Date(msgTs).getTime();
        if (ageMs > 6 * 3600 * 1000) return { status: 'fail', detail: `HEAD commit is ${Math.round(ageMs / 60000)} min old: claimed commit did not land` };
        return { status: 'pass', detail: `HEAD ${head.slice(0, 8)} committed ${Math.round(ageMs / 60000)} min ago` };
      }
      return { status: 'pass', detail: `repo reachable, HEAD ${head.slice(0, 8)}` };
    } catch (e) {
      return { status: 'unverifiable', detail: 'git not reachable from cwd: ' + e.message.split('\n')[0] };
    }
  }
