...
const { extractClaims, findPhantoms, deletionCommands, extractCoverageClaims } = require('./claims');
...
async function runAudit(sessionId, transcriptPath) {
  ...
  for (let i = 0; i < ledger.length; i++) {
    ...
    // fail. Only fs targets and only when the delete command names the exact path.
    const nt = entry.target ? String(entry.target).replace(/\\/g, '/').toLowerCase() : null;
    const intentionallyDeleted = nt != null && delCmds.some(cmd => cmd.includes(nt));
    let verdict;
