...
const { extractClaims, findPhantoms, deletionCommands, recursivelyDeletedDirs, underDeletedDir, extractCoverageClaims } = require('./claims');
...
async function runAudit(sessionId, transcriptPath) {
  ...
  for (let i = 0; i < ledger.length; i++) {
    ...
    const intentionallyDeleted = nt != null &&
      (delCmds.some(cmd => cmd.includes(nt)) || underDeletedDir(deletedDirs, nt));
    let verdict;
