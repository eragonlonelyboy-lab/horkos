...
function deletedExactPaths(transcriptPath) {
  ...
  for (const line of lines) {
    ...
    for (const c of msg.content) {
      if (c.type !== 'tool_use' || !c.input || typeof c.input.command !== 'string') continue;
      for (const seg of c.input.command.split(/&&|\|\||;|\n/)) {
        if (!DEL_VERB_RE.test(seg)) continue;
        for (const raw of seg.trim().split(/\s+/).slice(1)) {
          if (!raw || raw.startsWith('-')) continue;              // flags
          if (/[$*?<>|]/.test(raw)) continue;                     // globs, shell vars, redirects
          const t = raw.replace(/^["']|["']$/g, '').replace(/\\/g, '/').replace(/\/+$/, '').toLowerCase();
          if (!t || t === '.' || t === '..' || t === '/' || t === '~') continue;
          paths.add(t);
        }
...
module.exports = { extractClaims, findPhantoms, finalAssistantText, deletionCommands, recursivelyDeletedDirs, underDeletedDir, deletedExactPaths, deletedExactly, extractCoverageClaims, COVERAGE_ASSERT_RE };
