...
14. **A system noun as the SUBJECT of a descriptive verb** (dogfood 2026-07-11): "a fully finished ticket shows as 6 out of 9 done" was read as a Jira write, because "finished"/"done" hit the claim verb and "ticket" hit the jira hint. Fix: a noun (ticket/issue/page/test case/test run) directly followed by a display verb (shows/renders/displays/appears/reads) is masked as a UI-state description; an explicit system name elsewhere in the sentence still counts as a hint.
15. **A write verb time-stamped BEFORE this turn** (dogfood 2026-07-11): "the repo was clean and pushed before this; my fix sits there uncommitted" audited as a phantom push. Fix: an explicit prior-time marker (before this, previously, in an earlier session) adjacent to the verb reports prior state, not this turn's action; a bare stative claim with no marker still phantoms.
16. **Bare generic nouns do not name a system** (dogfood 2026-07-11): "branch on cards" (a dashboard feature) and "editing a ticket on disk" (a local file) were attributed to git and Jira. Fix: jira needs the word "jira" and git needs a git operation, not the bare nouns "branch"/"ticket"; explicit system names still attribute.
17. **A read-only capability inventory that DISCLAIMS writes** (dogfood 2026-07-15, executor build): a table row "TestRail: 29 read tools, no add/update/delete/close" audited as a TestRail write. The slash-delimited negated verb list had only its first verb masked, so "update" survived, hit the claim verb, and the column label "TestRail" hit the hint; the write-disclaimer produced the write-claim. Fix: the negated/offered coordination tail now chains slash- and comma-delimited verb lists, not just spaced conjunctions, so the whole disclaimer is masked. A real slash-joined claim with no negator still phantoms.
